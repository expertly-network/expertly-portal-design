...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2b. Hero Headline Rotator (Common -> Practitioner -> Client -> loop)
  initHeroRotator();

  // 2c. AI Search Section - functional search box
  initAISectionSearch();
...
/* ==========================================================================
   2c. AI SEARCH SECTION (functional search box, replaces hero search)
   ========================================================================== */
function initAISectionSearch() {
  const form = document.getElementById('ai-section-search-form');
  const field = document.getElementById('ai-section-search-field');
  if (!form || !field) return;

  const runSearch = (query) => {
    const q = (query || '').trim() || field.value.trim() || field.placeholder;
...
function initHeroRotator() {
  ...
  const slides = [
    { audience: 'common', html: "You Have Expertise.<br>Clients Have Needs.<br><span class=\"accent-text\">We Bring You Together.</span>" },
    { audience: 'practitioner', html: "You're Good at This.<br>Let the Right People<br><span class=\"accent-text\">Find Out.</span>" },
    { audience: 'client', html: "Stuck on Something?<br>Expert Help is<br><span class=\"accent-text\">Already Listed.</span>" }
  ];
  let idx = 0;
  const SLIDE_MS = 750; // matches the CSS transition duration
  const HOLD_MS = 4800;

  function showSlide(nextIdx) {
    const current = mount.querySelector('.hero-rotator-line');
    if (!current) return;

    const incoming = document.createElement('span');
    incoming.className = 'hero-rotator-line is-entering';
    incoming.setAttribute('data-audience', slides[nextIdx].audience);
    incoming.innerHTML = slides[nextIdx].html;
    mount.appendChild(incoming);

    // Force a reflow so the browser registers the "is-entering" (off to the
    // right) starting position before it's removed below - otherwise both
    // style changes get batched and the slide-in transition gets skipped.
    void incoming.offsetWidth;

    // Trigger both transitions on the same frame, so the outgoing line
    // slides out left while the incoming line slides in from the right,
    // at the same time, instead of one finishing before the next starts.
    current.classList.add('is-leaving');
    incoming.classList.remove('is-entering');

    setTimeout(() => current.remove(), SLIDE_MS);
  }
